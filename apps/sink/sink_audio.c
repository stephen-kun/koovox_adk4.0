...
{    
    ...
    {                
        ...
        /* mute control */
        VolumeSetHfpMicrophoneGain(priority, (theSink.profile_data[index].audio.gMuted ? VOLUME_MUTE_ON : VOLUME_MUTE_OFF));            
    }
}
    ...
    if(sink)
    {
        ...
        codec_settings = A2dpCodecGetSettings(theSink.a2dp_link_data->device_id[Index], theSink.a2dp_link_data->stream_id[Index]);
            ...
            if(codec_settings)
            {
                ...
                if (theSink.volume_levels->a2dp_volume[Index].main_volume == VOLUME_A2DP_MUTE_GAIN)
                {
                    VolumeUpdateMuteState(audio_mute_group_main, AUDIO_MUTE_ENABLE);
                }
                if (theSink.volume_levels->a2dp_volume[Index].aux_volume == VOLUME_A2DP_MUTE_GAIN)
                {
                ...
                VolumeSetAndUpdateMainAndAuxVolume(&theSink.volume_levels->a2dp_volume[Index]);
