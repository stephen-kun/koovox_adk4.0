...
void handleEventUsrSubwooferDeletePairing(void)
{
    ...
    if(deviceManagerGetAddrFromProfile(profile, &dev_addr))
    {
    	 ...
    	 * the protection flag.*/
    	ConnectionAuthSetPriorityDevice((const bdaddr *)&dev_addr.addr, FALSE);
...
void updateSwatVolumeNow(int16 new_volume)
{
    ...
    swat_volume = VolumeConvertStepsToDB(new_volume, &sinkVolumeGetGroupConfig(multi_channel_group_main), DSP_DB_SCALE);
    swat_volume /= 30;
