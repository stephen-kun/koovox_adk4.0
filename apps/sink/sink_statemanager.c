...
void stateManagerEnterIncomingCallEstablishState ( void )
{
    ...
    MessageCancelAll ( &theSink.task , EventSysPairingFail ) ;
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.IncomingRingPIO , pio_drive, TRUE) ;
...
void stateManagerEnterOutgoingCallEstablishState ( void )
{
    ...
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.OutgoingRingPIO , pio_drive, TRUE) ;
}
/****************************************************************************
NAME	
...
void stateManagerEnterActiveCallState ( void )   
{
    ...
    else
    {
        ...
        /*disable the ring PIOs if enabled*/
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.IncomingRingPIO , pio_drive, FALSE) ;
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.OutgoingRingPIO , pio_drive, FALSE) ;
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.CallActivePIO ,   pio_drive, TRUE) ;
...
void stateManagerPowerOn( void ) 
{
	...
	if(!checkForDUTModeEntry())
	{
        ...
        if(theSink.conf6->PIOIO.pio_outputs.PowerOnPIO != NO_PIO)
...
void stateManagerEnterLimboState ( void )
{
    ...
    /* Check if amplifier shut down is managed by PIO */
    if(theSink.features.AmplifierShutDownByPIO)
    {
...
void stateManagerAmpPowerControl(power_control_dir control){
    ...
    if( (control == POWER_UP) || (control == POWER_DOWN) ) 
    {
        ...
        if(theSink.conf6->PIOIO.pio_outputs.AudioMutePIO != NO_PIO)
            PioSetPio ( theSink.conf6->PIOIO.pio_outputs.AudioMutePIO , pio_drive, TRUE) ;
        
        /* Drive and set the run time logic state of the audio amplifier power pin */
        if( theSink.conf6->PIOIO.pio_outputs.PowerOnPIO != NO_PIO)
        {
            PioSetPio(theSink.conf6->PIOIO.pio_outputs.PowerOnPIO , pio_drive, (control == POWER_DOWN) ? FALSE : TRUE ) ;
            theSink.audioAmpPowerPin = (control == POWER_DOWN) ? FALSE : TRUE ;
        }    
        /* Reset Mute Signal now as AMP is already switched off */
        if(theSink.conf6->PIOIO.pio_outputs.AudioMutePIO != NO_PIO)
            PioSetPio ( theSink.conf6->PIOIO.pio_outputs.AudioMutePIO , pio_drive, FALSE) ;
    }
}    
/****************************************************************************
NAME	
...
void stateManagerUpdateLimboState ( void ) 
{
    ...
    if(power_off_request)    
    {
        ...
        PioSetPio(theSink.conf6->PIOIO.pio_outputs.PowerOnPIO , pio_drive, FALSE) ;
...
static void stateManagerResetPIOs ( void )
{
    ...
    PioSetPio ( theSink.conf6->PIOIO.pio_outputs.CallActivePIO ,   pio_drive, FALSE) ;
