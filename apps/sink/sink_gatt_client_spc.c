...
static void gattSpClientServiceInitialised(const GSPC_T *gspc, gatt_scan_params_client_status status)
{
    ...
    for (index = 0; index < GATT_CLIENT.number_connections; index++)
    {
        ...
        if (data && (data->spc == gspc))
        {
            ...
            {
                gattClientRemoveDiscoveredService(&GATT_CLIENT.connection[index], gatt_client_spc);
            }
...
void sinkGattSpClientSetupAdvertisingFilter(void)
{
    ...
    ConnectionBleAddAdvertisingReportFilter(AD_TYPE_SERVICE_UUID_16BIT_LIST, sizeof(spc_ble_advertising_filter), sizeof(spc_ble_advertising_filter), spc_ble_advertising_filter);
}
