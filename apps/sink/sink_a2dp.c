...
typedef struct
{
    ...
    */
    static const codec_list_element codecList[] = 
    {
        ...
        {APTX_SPRINT_CODEC_BIT, STD_SNK, &aptx_sprint_sep, (TaskData *)&csr_aptx_acl_sprint_decoder_plugin},
#endif
        /* High quality Sink SEPs */
        {APTX_CODEC_BIT, STD_SNK, &aptx_sep_snk, (TaskData *)&csr_aptx_decoder_plugin},
        {AAC_CODEC_BIT, STD_SNK, &aac_sep_snk, (TaskData *)&csr_aac_decoder_plugin},
        {MP3_CODEC_BIT, STD_SNK, &mp3_sep_snk, (TaskData *)&csr_mp3_decoder_plugin},
#ifdef ENABLE_PEER
#ifdef PEER_AS
        /* ShareMe Source SEPs */
        {APTX_CODEC_BIT, STD_SRC, &aptx_sep_src, (TaskData *)&csr_aptx_decoder_plugin},
        {AAC_CODEC_BIT, STD_SRC, &aac_sep_src, (TaskData *)&csr_aac_decoder_plugin},
        {MP3_CODEC_BIT, STD_SRC, &mp3_sep_src, (TaskData *)&csr_mp3_decoder_plugin},
#endif
#ifdef PEER_TWS
        ...
        {APTX_CODEC_BIT, TWS_SRC, &tws_aptx_sep_src, (TaskData *)&csr_tws_aptx_decoder_plugin},
        {APTX_CODEC_BIT, TWS_SNK, &tws_aptx_sep_snk, (TaskData *)&csr_tws_aptx_decoder_plugin},
        {AAC_CODEC_BIT, TWS_SRC, &tws_aac_sep_src, (TaskData *)&csr_tws_aac_decoder_plugin},
        {AAC_CODEC_BIT, TWS_SNK, &tws_aac_sep_snk, (TaskData *)&csr_tws_aac_decoder_plugin},
        {MP3_CODEC_BIT, TWS_SRC, &tws_mp3_sep_src, (TaskData *)&csr_tws_mp3_decoder_plugin},
        {MP3_CODEC_BIT, TWS_SNK, &tws_mp3_sep_snk, (TaskData *)&csr_tws_mp3_decoder_plugin},
#endif
#endif
#ifdef INCLUDE_FASTSTREAM                
        /* Faststream Sink SEP */
        {FASTSTREAM_CODEC_BIT, STD_SNK, &faststream_sep, (TaskData *)&csr_faststream_sink_plugin},
#endif
#endif
#ifdef ENABLE_PEER
#ifdef PEER_TWS
        ...
        {SBC_CODEC_BIT, TWS_SRC, &tws_sbc_sep_src, (TaskData *)&csr_tws_sbc_decoder_plugin},
        {SBC_CODEC_BIT, TWS_SNK, &tws_sbc_sep_snk, (TaskData *)&csr_tws_sbc_decoder_plugin},
#endif      
        /* Mandatory SBC Source SEP for both ShareMe and TWS */
        {SBC_CODEC_BIT, STD_SRC | TWS_SRC, &sbc_sep_src, (TaskData *)&csr_sbc_decoder_plugin},
#endif
        /* Mandatory SBC Sink SEP */
        {SBC_CODEC_BIT, STD_SNK | TWS_SNK, &sbc_sep_snk, (TaskData *)&csr_sbc_decoder_plugin}
    };
...
static void handleA2dpCodecConfigureIndFromPeer (A2DP_CODEC_CONFIGURE_IND_T* ind)
{
    ...
    case RELAY_SOURCE_ANALOGUE:
    {
        ...
        modifyCodecSettings(codec_settings, sample_freq);
        break;
    }    
    ...
    case RELAY_SOURCE_A2DP:
        ...
        {
            uint16 device_id = theSink.a2dp_link_data->device_id[priority];
            uint16 stream_id = theSink.a2dp_link_data->stream_id[priority];
...
#endif
                    {
                        VolumeModifyAndUpdateTWSDeviceTrim(dir, tws_device, &theSink.volume_levels->a2dp_volume[index]);
                        return TRUE;
                    }
                }
            }
...
/* Only certain TWS use cases require us to attempt to pause source devices */
    ...
    if(theSink.a2dp_link_data->SuspendState[id] == a2dp_not_suspended)
    {
        ...
        if((theSink.a2dp_link_data->peer_device[id] == remote_device_nonpeer) && a2dpGetPeerIndex(&peerIndex) && sinkAvrcpGetIndexFromBdaddr(&theSink.a2dp_link_data->bd_addr[id], &avrcpIndex, TRUE))
        {
            if (theSink.features.TwsSingleDeviceOperation  && (theSink.a2dp_link_data->remote_peer_status[peerIndex] & PEER_STATUS_IN_CALL))
            {                
...
void InitA2dp(void)
{
    ...
    theSink.a2dp_link_data->a2dp_audio_connect_params.speaker_pio = theSink.conf6->PIOIO.pio_outputs.DeviceAudioActivePIO ;
#endif/*ENABLE_SOUNDBAR*/
...
void handleA2DPSignallingConnected(a2dp_status_code status, uint16 DeviceId, bdaddr SrcAddr, bool locally_initiated)
{
    ...
    if (status != a2dp_success)
    {
        ...
        if(isTWSDeviceAvailable(DeviceId))
        {
...
#endif
            {
               ...
               set EQ mode as it might be that the device has a SCO routed instead */
                if(theSink.routed_audio == sink)
                {
                    ...
                    if((theSink.volume_levels->a2dp_volume[Id].main_volume) == VOLUME_A2DP_MUTE_GAIN)
                    {
                        VolumeUpdateMuteState(audio_mute_group_main, AUDIO_MUTE_ENABLE);
                    }
                    if((theSink.volume_levels->a2dp_volume[Id].aux_volume) == VOLUME_A2DP_MUTE_GAIN)
                    {
            ...
            if(stateManagerGetState() == deviceA2DPStreaming)
            {
                ...
                if((theSink.PartyModeEnabled)&&(theSink.features.PartyMode))
                {
                    /* start a timer when music is suspended in party mode, if no music is played before the timeout
                       occurs the device will get disconnected to allow other devices to connect, currently set to 
                       2 minutes */
                    MessageSendLater(&theSink.task,(EventSysPartyModeTimeoutDevice1 + Id),0,PARTYMODE_MUSIC_TIMEOUT);                 
                }
#endif
            }
    ...
    for_all_a2dp(i)
    {
        ...
        if(theSink.a2dp_link_data->connected[i])
        {
            ...
            if(BdaddrIsSame(&theSink.a2dp_link_data->bd_addr[i], bd_addr))
            {
                ...
                uint16 lOldVol = theSink.volume_levels->a2dp_volume[i].main_volume;
                uint16 a2dp_vol_max = sinkVolumeGetGroupConfig(multi_channel_group_main).no_of_steps - 1;
                
                if(a2dp_volume > a2dp_vol_max)
                    a2dp_volume = a2dp_vol_max;
                
                theSink.volume_levels->a2dp_volume[i].main_volume = a2dp_volume;
                ...
                {
                    /* Only generate max/min tone if volume has changed */
                    if(lOldVol != a2dp_volume)
                    {
...
#ifdef ENABLE_PEER
            handlePeerQualificationReconfigureCfm(((A2DP_MEDIA_RECONFIGURE_CFM_T*)message)->device_id,
                              ((A2DP_MEDIA_RECONFIGURE_CFM_T*)message)->stream_id,
                              ((A2DP_MEDIA_RECONFIGURE_CFM_T*)message)->status);
#endif
            break;
...
{
    uint16 device_id = theSink.a2dp_link_data->device_id[a2dp_link];
    uint16 stream_id = theSink.a2dp_link_data->stream_id[a2dp_link];
