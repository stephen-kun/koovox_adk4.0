...
{    
    ...
    {
        /* If sound bar amplifier shut down is managed by PIO, first check the logic state of amplifier power pin */
    ...
    if (!VolumeAllOutputsMuted() || (pEvent == EventSysMuteReminder)
                                 || (pEvent == EventUsrAuxOutVolMuteOn)
                                 || (pEvent == EventUsrMainOutVolMuteOn))
    {
     ...
     while ( theSink.conf7->gEventTones [i].tone != TONE_NOT_DEFINED )
     {
      ...
      if (theSink.conf7->gEventTones [i].event == lEvent )
      {
                /* turn on audio amp */
...
int16 TonesGetToneVolumeInDb(audio_plugin_mch_group_t group)
{
    ...
    return VolumeConvertStepsToDB(tone_steps, vol_conf, DSP_DB_SCALE);
}
