...
static bool handleAvrcpVendorUniquePassthroughInd (AVRCP_PASSTHROUGH_IND_T *ind)
{
    ...
    case AVRCP_PEER_CMD_UPDATE_TRIM_VOLUME:     
        ...
        case AVRCP_PEER_CMD_UPDATE_BATTERY_LEVEL:
            ...
            {
                uint16 battery_level = ind->op_data[2] << 8 | ind->op_data[3];
                if(!peerUpdateBatteryLevel(battery_level))
                {
...
static void handleAvrcpSetAbsVolInd(AVRCP_SET_ABSOLUTE_VOLUME_IND_T *msg)
{
    ...
    {        
        ...
        a2dp_volume = ((msg->volume * sinkVolumeGetGroupConfig(multi_channel_group_main).no_of_steps)/(AVRCP_MAX_ABS_VOL));
...
**************************************************************************/
void sinkAvrcpVolumeStepChange(volume_direction operation, uint16 step_change, uint16 no_of_steps)
{
    uint16 Index = sinkAvrcpGetActiveConnection();
    uint16* avrcp_vol = &theSink.avrcp_link_data->absolute_volume[Index];
    
    /* Make AVRCP step change proportional to the step change in local volume */
    uint16 avrcp_step_change = (step_change * AVRCP_MAX_ABS_VOL * AVRCP_ABS_VOL_STEP_CHANGE) / 
                               (no_of_steps - 1);
    ...
    {            
        if (operation == increase_volume)
        {
            ...
            *avrcp_vol += avrcp_step_change;
            
            /* zero step change indicates the max volume has been reached */            
            if(((*avrcp_vol) + avrcp_step_change > AVRCP_MAX_ABS_VOL) || !(avrcp_step_change))
            {
                *avrcp_vol = AVRCP_MAX_ABS_VOL;
            }
        ...
        else if (operation == decrease_volume)
        {
            ...
            if ((avrcp_step_change) && ((*avrcp_vol) - avrcp_step_change >= avrcp_step_change))
                *avrcp_vol -= avrcp_step_change;
            else
                *avrcp_vol = 0;
