...
static bool audioSoundbarRouting(audio_sources requested_source,
                                audio_source_status *lAudioStatus);
#else
static bool audioHeadsetRouting(audio_sources requested_source,
                                audio_source_status *lAudioStatus);
#endif
...
#if defined(ENABLE_USB) || defined(ENABLE_WIRED)
...
{       
    ...
    AUD_DEBUG(("AUD: audioHandleRouting %x", (uint16)theSink.routed_audio));
    ...
    routing_success = audioHeadsetRouting(requested_source, lAudioStatus);
#else
    routing_success = audioSoundbarRouting(requested_source, lAudioStatus);
#endif
                                ...
                                audio_source_status *lAudioStatus)
{
    ...
    else if(sinkCheckPartyModeAudio(requested_source, lAudioStatus))                
    {
    ...
    return TRUE; /* for compatibility with audioSoundbarRouting() */
}
...
bool audioRouteSource(audio_sources source, audio_source_status * lAudioStatus)
{
    ...
    if(source != audio_source_none)
    {
        ...
        switch(source)
        {
            ...
            case audio_source_FM:
#ifdef ENABLE_FM
                if(audioRouteFMIfAvailable(lAudioStatus))
                    status = TRUE;
#endif
            break;
            ...
            case audio_source_ANALOG:
#ifdef ENABLE_WIRED  
                if(audioRouteAnalogIfAvailable(lAudioStatus))
                    status = TRUE;
#endif                
            break;
            ...
            case audio_source_SPDIF:
#ifdef ENABLE_WIRED  
                if(audioRouteSpdifIfAvailable(lAudioStatus))
                    status = TRUE;
#endif                
            break;
            ...
            case audio_source_USB:
#ifdef ENABLE_USB            
                if(audioRouteUsbIfAvailable(lAudioStatus))    
                    status = TRUE;
#endif                
            break;
...
void audioUpdateDisplayAmp(Sink audio_routed, audio_source_status * lAudioStatus)
{
    ...
    {           
        ...
        if(audio_routed)
            PioSetPio ( theSink.conf6->PIOIO.pio_outputs.DeviceAudioActivePIO , pio_drive, TRUE) ;            
    }
...
bool audioRouteAncIfAvailable(audio_source_status * lAudioStatus)
{
    ...
    if(sinkAncIsEnabled())
    {
        ...
        if(lAudioStatus->audio_routed)
        {
            ...
            {
                AUD_DEBUG(("AUD: ANC audio but not ANC, suspend/disconnect\n"));
    ...
    AUD_DEBUG(("AUD: ANC audio routed=[%u]\n", audio_routed));
...
{
     /* only allow source switching when powered on and if not currently in the process of
        switching audio sources to reduce system loading */
     if ((stateManagerGetState() != deviceLimbo) && (!IsAudioBusy()))
     {
            /* switch to required source */
            audioSwitchToAudioSource(source);
            ...
            /* switched source sucessfully */
            return TRUE;
     }                
     else
     {
         /* source not switched to that requested due to currently processing a different   
            source request */
         return FALSE;
     }
}
