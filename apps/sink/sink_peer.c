...
#endif  /* ENABLE_PEER_BATTERY_LEVEL */
...
bool RequestRemotePeerServiceRecord (Task task, const bdaddr *bd_addr)
{
    ...
    else
    {
        ...
        if ( expandUuid128((uint16 *)&search_pattern[3]) )
        {
            ...
            ConnectionSdpServiceSearchAttributeRequest(task, bd_addr, 32, sizeof(peer_device_search_pattern), (uint8 *)search_pattern, sizeof(peer_device_attr_list), peer_device_attr_list);
            return TRUE;
        }
        else
        {
...
**************************************************************************/
static void updateMuteState(void)
{
    ...
    else if((theSink.peer.current_state == RELAY_STATE_STREAMING) && 
       (theSink.peer.target_state == RELAY_STATE_STREAMING))
    {
        VolumeUpdateMuteStatusAllOutputs(FALSE);
    }
...
#ifdef ENABLE_USB
    if (((theSink.features.PeerSource == RELAY_SOURCE_USB) || (theSink.features.PeerSource == RELAY_SOURCE_NULL)) && (usbAudioIsAttached() && theSink.usb->spkr_active && (theSink.usb->audio_suspend_state == usb_audio_suspend_none)))
    ...
    if(!peerQualificationAdvanceRelayState(relay_event))
    {
        switch(relay_event)
        {
            ...
            {   /* Route relay stream */
                ...
                PEER_DEBUG(("PEER: Issue SOURCE START RESP\n"));
            }
            break;
...
void peerUpdateLocalStatusChange (PeerStatusChange peer_status_change)
{
    ...
    if (a2dpGetPeerIndex(&peer_id))
    {
        ...
        updateMuteState(); /* update mute state, if required */
    }
}
