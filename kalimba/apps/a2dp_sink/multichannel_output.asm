...
// Part of ADK 4.0
//
// $Change: 2039117 $  $DateTime: 2014/11/25 11:16:25 $
// *****************************************************************************
...
.MODULE $M.multi_chan_output;
   ...
   // Table of ports for all possible wired outputs
   ...
   .VAR $interface_map_struc[$INTERFACE_MAP_SIZE]=
      0,                                                             // Bitmask for wired output channels with connected ports
      0,                                                             // Bitmask for DAC or I2S channels
      0,                                                             // Bitmask for Chain 1 channels
      0;                                                             // Bitmask for SPDIF channels
   ...
   TEMPLATE_MUTE_OP(chain0_ch0_mute_op, chain0_ch0_dc_remove_op, DYNAMIC, 0, 0)
   ...
   TEMPLATE_MUTE_OP(chain0_ch1_mute_op, chain0_ch1_dc_remove_op, DYNAMIC, 1, 1)
   TEMPLATE_DC_REMOVE_OP(chain0_ch1_dc_remove_op, chain0_anc_switch_op, 1, 1)

   // Select standard operation or ANC operation
   TEMPLATE_SWITCH_ALT_OP(chain0_anc_switch_op, chain0_ch0_resamp_switch_op, chain0_ch0_dither_switch_op, chain0_anc_enable, $ANC_MASK)
   ...
   TEMPLATE_IIR_RESAMPLEV2_OP(chain0_ch0_resamp_op, chain0_ch1_resamp_switch_op, 0, DYNAMIC, DYNAMIC, -8, 0)
   ...
   TEMPLATE_IIR_RESAMPLEV2_OP(chain0_ch1_resamp_op, cbops.NO_MORE_OPERATORS, 1, DYNAMIC, DYNAMIC, -8, 0)
   ...
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch0_dither_and_shift_op, chain0_ch1_dither_switch_op, 0, DYNAMIC, DYNAMIC)
   ...
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch1_dither_and_shift_op, chain0_ch2_switch_op, 1, DYNAMIC, DYNAMIC)
   ...
   TEMPLATE_MUTE_OP(chain0_ch2_mute_op, chain0_ch2_dc_remove_op, DYNAMIC, 2, 2)
   TEMPLATE_DC_REMOVE_OP(chain0_ch2_dc_remove_op, chain0_ch2_dither_and_shift_op, 2, 2)
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch2_dither_and_shift_op, chain0_ch3_switch_op, 2, DYNAMIC, DYNAMIC)
   ...
   TEMPLATE_MUTE_OP(chain0_ch3_mute_op, chain0_ch3_dc_remove_op, DYNAMIC, 3, 3)
   TEMPLATE_DC_REMOVE_OP(chain0_ch3_dc_remove_op, chain0_ch3_dither_and_shift_op, 3, 3)
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch3_dither_and_shift_op, chain0_ch4_switch_op, 3, DYNAMIC, DYNAMIC)
   ...
   TEMPLATE_MUTE_OP(chain0_ch4_mute_op, chain0_ch4_dc_remove_op, DYNAMIC, 4, 4)
   TEMPLATE_DC_REMOVE_OP(chain0_ch4_dc_remove_op, chain0_ch4_dither_and_shift_op, 4, 4)
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch4_dither_and_shift_op, chain0_ch5_switch_op, 4, DYNAMIC, DYNAMIC)
   ...
   TEMPLATE_MUTE_OP(chain0_ch5_mute_op, chain0_ch5_dc_remove_op, DYNAMIC, 5, 5)
   TEMPLATE_DC_REMOVE_OP(chain0_ch5_dc_remove_op, chain0_ch5_dither_and_shift_op, 5, 5)
   TEMPLATE_DITHER_AND_SHIFT_OP(chain0_ch5_dither_and_shift_op, cbops.NO_MORE_OPERATORS, 5, DYNAMIC, DYNAMIC)
   ...
   // Channel 0
   TEMPLATE_MUTE_OP(chain1_ch0_mute_op, chain1_ch0_dc_remove_op, DYNAMIC, 0, 0)
   ...
   TEMPLATE_MUTE_OP(chain1_ch1_mute_op, chain1_ch1_dc_remove_op, DYNAMIC, 1, 1)
   ...
   // Channel 0
   TEMPLATE_DITHER_AND_SHIFT_OP(chain1_ch0_dither_and_shift_op, chain1_ch1_switch2_op, 0, DYNAMIC, DYNAMIC)
   ...
   // Channel 1
   TEMPLATE_DITHER_AND_SHIFT_OP(chain1_ch1_dither_and_shift_op, cbops.NO_MORE_OPERATORS, 1, 3, DYNAMIC)
...
.MODULE $M.multi_chan_config_cbops_copy_strucs;
   ...
   if Z jump skip_chain0_config;
      ...
      // Check if ANC is enabled and, if so, at what output rate
      r3 = M[$M.multi_chan_output.chain0_anc_enable];

      // Output rate before ANC resampling
      r2 = M[$current_dac_sampling_rate];

      // Check the ANC rate (if no ANC then defaultt to 96kHz filter config)
      null = r3 - $ANC_192K;
      if Z jump anc_192k;

         // ANC @ 96kHz
         // Configure the ANC resampler filter (44.1kHz or 48kHz ->96kHz)
         r0 = $M.iir_resamplev2.Up_320_Down_147.filter;                             // 44100 -> 96000
         r1 = $M.iir_resamplev2.Up_2_Down_1.filter;                                 // 48000 -> 96000
         null = r2 - 48000;
         if Z r0 = r1;
      ...
      anc_192k:
         // ANC @ 192kHz
         // Configure the ANC resampler filter (44.1kHz or 48kHz ->192kHz)
         r0 = $M.iir_resamplev2.Up_640_Down_147_low_mips.filter;                    // 44100 -> 192000
         r1 = $M.iir_resamplev2.Up_4_Down_1.filter;                                 // 48000 -> 192000
         null = r2 - 48000;
         if Z r0 = r1;
         TEMPLATE_OP_CONFIG($M.multi_chan_output.chain0_ch0_resamp_op, param, $iir_resamplev2.FILTER_DEFINITION_PTR_FIELD, r0)
         TEMPLATE_OP_CONFIG($M.multi_chan_output.chain0_ch1_resamp_op, param, $iir_resamplev2.FILTER_DEFINITION_PTR_FIELD, r0)
...
.MODULE $M.multi_chan_build_cbops_copy_struc;
   ...
   r0 = $M.multi_chan_output.wired_out_port_table; // Src addr for the wired output port table
   r10 = $MULTI_CHAN_MAX_WIRED_OUT_CHANNELS;       // Maximum number of wired output channels
...
//
// DESCRIPTION:
//    Set the output handler timer according to the ANC setting
//
// INPUTS:
//    none
//
...
.MODULE $M.set_output_handler_timer;
   ...
   r0 = TMR_PERIOD_AUDIO_COPY;                           // Default timer period
   r1 = ANC_192K_TMR_PERIOD_AUDIO_COPY;                  // Timer period for ANC @192kHz

   // Use lower period for ANC @ 192kHz
   null = r3 - $ANC_192K;
   if Z r0 = r1;
