...
void gattClientSendInitSuccessCfm(GGATTC *gatt_client, uint16 service_changed_handle)
{
    ...
    message->status = gatt_client_status_success;
...
void gattClientSendInitDiscoveryErrorCfm(GGATTC *gatt_client, uint16 service_changed_handle)
{
    ...
    message->gatt_client = gatt_client;
    message->service_changed_handle = service_changed_handle;
    message->status = gatt_client_status_discovery_error;
                                    ...
                                    const gatt_client_device_data_t *device_data)
{
    ...
    if (GattManagerRegisterClient(&registration_params) == gatt_manager_status_success)
    {
        ...
        if (device_data)
        {
            ...
            gattClientSendInitSuccessCfm(gatt_client, device_data->service_changed_handle);
        }
        else
        {
