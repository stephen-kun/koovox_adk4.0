...
void hfpServicesInit(hfp_profile profile, bool multipoint)
{
    ...
    if(supportedProfileIsHfp(profile))
    {
        ...
        service = hfpServiceInit(service, handsfree, HFP_DEFAULT_CHANNEL);
        
        /* Create a second HFP service entry if multipoint is enabled */
        if(multipoint)
            service = hfpServiceInit(service, handsfree, HFP_DEFAULT_CHANNEL_2);
    }
    ...
    if(supportedProfileIsHsp(profile))
    {
        ...
        service = hfpServiceInit(service, headset, HSP_DEFAULT_CHANNEL);
        
        /* Create a second HSP service entry if multipoint is enabled */
        if(multipoint)
            service = hfpServiceInit(service, headset, HSP_DEFAULT_CHANNEL_2);
    }
}
