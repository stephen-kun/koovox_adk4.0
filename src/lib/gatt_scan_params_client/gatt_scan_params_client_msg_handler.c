...
static void writeScanIntervalWindow(GSPC_T *scan_params_client)
{
    ...
    value[2] = scan_params_client->scan_window & 0x00ff;
    value[3] = scan_params_client->scan_window >> 8;
...
*/
static void handleScanParamsGattMsg(Task task, MessageId id, Message msg)
{
     /* GATT unrecognised messages */
    GATT_SCAN_PARAMS_CLIENT_DEBUG_PANIC(("Unknown Message received from GATT lib\n"));
}
...
static void handleScanParamsGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        case GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM:
        {
            ...
            handleScanParamsDiscoverCharCfm(scan_params_client,(GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T*)payload);
        }
        break;
        ...
        {
            /* Characteristic handle discovery */
            handleScanParamsDiscoverCharDescCfm(scan_params_client, (GATT_MANAGER_DISCOVER_ALL_CHARACTERISTIC_DESCRIPTORS_CFM_T *)payload);
        }
        break;

        case GATT_MANAGER_WRITE_CHARACTERISTIC_VALUE_CFM:
        {
            handleScanRefreshEnableNotificationResp(scan_params_client, (GATT_MANAGER_WRITE_CHARACTERISTIC_VALUE_CFM_T *)payload);
        }
        break;
        ...
        {
            handleScanRefreshNotification(scan_params_client, (GATT_MANAGER_REMOTE_SERVER_NOTIFICATION_IND_T *)payload);
        }
        break;
...
*/
static void  handleScanParamsInternalMsg(Task task, MessageId id, Message msg)
{
    GSPC_T *scan_params_client = (GSPC_T *)task;
...
void scanParamsClientMsgHandler(Task task, MessageId id, Message payload)
{
    ...
    {
        handleScanParamsGattMsg(task, id, payload);
    }
    ...
    {
        handleScanParamsInternalMsg(task, id, payload);
    }
    else
    {
