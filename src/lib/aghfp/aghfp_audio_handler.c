...
static void continueAudioConnectRequest(AGHFP *aghfp)
{
    ...
    {  
        ...
        /* when trying EV3 S1(in caseCVSD) or T1(in case of mSBC) settings, substitute the S1/T1 settings instead of user supplied settings */
        if(aghfp->audio_packet_type_to_try == (sync_ev3 | sync_all_edr_esco))
        {
            if(aghfp->audio_params.voice_settings == sync_air_coding_transparent)
            {
                /* Issue Synchronous connect request to connection lib with T1 safe settings in case of mSBC negotiation failure */
                ConnectionSyncConnectRequest(&aghfp->task, aghfp->rfcomm_sink, &default_t1_sync_config_params);
            }
            else
            {
