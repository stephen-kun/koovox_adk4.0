            ...
            const imm_alert_cfm_mesage_type cfmType)
{
    ...
    {
        send_imm_alert_client_cfm(imm_alert_client,cid,gatt_imm_alert_client_status_no_connection,cfmType);
    }
    else
    {
        /* Any other failure; inform application with failure status */
        send_imm_alert_client_cfm(imm_alert_client,cid,gatt_imm_alert_client_status_failed,cfmType);
    }
}
    ...
    if(imm_alert_client != NULL)
    {
        if(char_cfm->status == gatt_status_success)
        {
            ...
            if((char_cfm->uuid_type == gatt_uuid16)&&(char_cfm->uuid[0] == IMM_ALERT_LEVEL_CHAR_UUID))
            {
                ...
                send_imm_alert_client_cfm(imm_alert_client,char_cfm->cid,
              ...
              const GATT_MANAGER_WRITE_WITHOUT_RESPONSE_CFM_T *const write_cfm)
{
    if(imm_alert_client != NULL)
    {
        ...
        {
            /* Report success to application */
            send_imm_alert_client_cfm(imm_alert_client,write_cfm->cid,
...
static void handle_imm_alert_client_gatt_manager_msg(Task task, MessageId id, Message msg)
{
    ...
    switch(id)
    {
        ...
        case GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM:
        {
            ...
            handle_imm_alert_discover_char_cfm(imm_alert_client,(GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T*)msg);
        }
        break;
        ...
        {
            /* Write Confirmation */
            handle_imm_alert_write_response_cfm(imm_alert_client,(GATT_MANAGER_WRITE_WITHOUT_RESPONSE_CFM_T*)msg);
        }
        break;
...
*/
static void handle_imm_alert_ext_message(Task task, MessageId id, Message msg)
{
     /* GATT unrecognised messages */
     GATT_IMM_ALERT_CLIENT_DEBUG_PANIC(("Unknown Message received from GATT lib \n"));
}

/***************************************************************************
NAME
    handle_imm_alert_inernal_message

DESCRIPTION
   Handles immediate Alert Service internal messages 
*/
static void  handle_imm_alert_inernal_message(Task task, MessageId id, Message msg)
{
    GIASC_T *const imm_alert_client = (GIASC_T*)task;
    
    GATT_IMM_ALERT_CLIENT_DEBUG_INFO(("handle_imm_alert_inernal_messge(%d) \n",id));
    
    switch(id)
    {
        /* Connect Request */
        case IMM_ALERT_CLIENT_INETRNAL_MSG_CONNECT:
        {
        ...
        {
            imm_alert_client_set_alert_level(imm_alert_client,(IMM_ALERT_CLIENT_INTERNAL_MSG_SET_ALERT_T*)msg);
        }
        break;
