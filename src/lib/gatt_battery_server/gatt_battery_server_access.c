                                    ...
                                    const uint8 *value)
{
    if (!GattManagerServerAccessResponse(task, cid, handle, result, size_value, value))
    {
		...
		GATT_BATTERY_DEBUG(("Couldn't send GATT access response\n"));
    }
}
...
static void batteryServiceAccess(GBASS *battery_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
	...
	GATT_BATTERY_DEBUG(("batteryServiceAccess:%x\n", access_ind->flags));
...
static void batteryLevelAccess(GBASS *battery_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
	...
	GATT_BATTERY_DEBUG(("batteryLevelAccess:%x\n", access_ind->flags));
...
static void batteryClientConfigAccess(GBASS *battery_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
	...
	GATT_BATTERY_DEBUG(("batteryClientConfigAccess:%x\n", access_ind->flags));
...
static void batteryPresentationAccess(GBASS *battery_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
	...
	GATT_BATTERY_DEBUG(("batteryPresentationAccess:%x\n", access_ind->flags));
...
void handleBatteryAccess(GBASS *battery_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
    switch (access_ind->handle)
    {
		...
		case HANDLE_GATT_TEST:
		{
			GATT_BATTERY_DEBUG(("HANDLE_GATT_TEST:flags=%x, size=%d\n", access_ind->flags, access_ind->size_value));
			
			if (access_ind->flags & ATT_ACCESS_WRITE)
			{  
				sendBatteryAccessRsp((Task)&battery_server->lib_task, access_ind->cid, access_ind->handle, gatt_status_success, 0, 0);
			}
        ...
        default:
        {
			GATT_BATTERY_DEBUG(("handleBatteryAccess:%x\n", access_ind->handle));
