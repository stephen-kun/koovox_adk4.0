    ...
    gatt_hid_client_status status)
{
    ...
    memset(message, 0, sizeof(GATT_HID_CLIENT_INIT_CFM_T));
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_send_discovery_cfm(),status = %x,cid =%x,rpid =%d  \n",status,cid,message->num_report_id));
               ...
               uint16 *start_handle)
{
    ...
    hid_instance->characterisitc_handle = INVALID_HID_HANDLE;
    ...
    if(hid_instance->char_descriptor_mask>0)
    {
        ...
        {
            GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_get_char_handles(), num Report ; %d\n",hid_instance->num_reports_available));
           ...
           GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_get_char_handles(), read handle count ; %d\n",hid_instance->read_report_handle_count));
        }
        else
        {
            /* Error, Wrong Bit Mask Value */
           GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_get_char_handles(),Wrong Bit Mask \n"));
        }
        /* Clear The bit mask */
        if(clear_bit_mask > 0)
        {
        ...
        hid_instance->characterisitc_handle = *start_handle;
    }
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_get_char_handles(), Exit: start_handle = %x\n",*start_handle));
}
            ...
            uint16* end_handle)
{
    ...
    for(count = 0;count < hid_instance->char_uuid_count;count++)
    {
       ...
       GATT_HID_CLIENT_DEBUG_INFO(("Current = %x, count = %d start_handle = %x\n",current,count,start_handle));
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_get_char_end_handles(), Exit:status = %d Handle = %x\n",status,current-1));
}
                ...
                uint16 cid)
{
    ...
    if(start_handle && end_handle)
    {
        ...
        GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_discover_next_char_descriptor(), Succes:Start :%x,End :%x \n",start_handle,end_handle));
        return TRUE;
    }
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_discover_next_char_descriptor(), Failed:Start :%x,End :%x \n",start_handle,end_handle));
    return FALSE;
}
        ...
        if(hid_client->boot_mode_supported)
        {
            for (index = 0; index < max_ccd_num; index ++)
            {
                ...
                if ((!hid_client->ccd_handles[index].ccd)&& (hid_client->num_boot_ccd < hid_instance->max_bootmode_ccd))
                {
                    ...
                    hid_client->ccd_handles[index].ccd_type = gatt_hid_boot_ccd;
                    hid_client->ccd_handles[index].characterisitc_handle = hid_instance->characterisitc_handle;
                    hid_client->num_boot_ccd++;
                    GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_ccd_handles(),handle = %x,numBootCCD= %x \n",handle,hid_client->num_boot_ccd));
                    break;
                }
            }
        }
    ...
    else
    {
        ...
        for (index = 0; index < max_ccd_num; index ++)
        {
            if ((!hid_client->ccd_handles[index].ccd)&&(hid_client->num_report_ccd <hid_instance->max_reportmode_ccd ))
            {
                /* Free CCD found, update and exit */
                hid_client->ccd_handles[index].ccd = handle;
                hid_client->ccd_handles[index].ccd_type = gatt_hid_report_ccd;
                hid_client->ccd_handles[index].characterisitc_handle = hid_instance->characterisitc_handle;
                hid_client->num_report_ccd++;
                GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_ccd_handles(),handle = %x,numReportCCD=%x \n",handle,hid_client->num_report_ccd));
                break;
            }
        }
    }
                 ...
                 uint16 handle)
{
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_characteristic_handles(),UUID : %x, handle : %x\n",uuid,handle));
    ...
    switch(uuid)
    {
        ...
        case GATT_HID_REPORT_UUID:
        {
            ...
            GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_characteristic_handles(),NumReports : %d\n",hid_instance->num_reports_available));
        ...
        default:
        {
           ...
           GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_characteristic_handles(),UnKnwnHandle\n"));
        }
        break;
    }
     GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_characteristic_handles(),Exit \n"));
}
                ...
                uint16 handle)
{
    ...
    switch (uuid)
    {
        ...
        case GATT_HID_REPORT_REFERENCE_UUID:
        {
           ...
           GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_char_descriptors(),numreportId %d \n",hid_instance->num_report_ids));
        }
        break;
        ...
        case GATT_HID_EXT_REPORT_REFERENCE_UUID:
        {
            ...
            GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_char_descriptors(),ext ref handle = %x \n",handle));
        ...
        case GATT_HID_CHARACTERISTIC_CONFIGURATION_UUID:
        {
            ...
            */
            GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_char_descriptors(),StoreCCD Handle=%x \n",handle));
        ...
        {
           /* Ignore */
           GATT_HID_CLIENT_DEBUG_INFO(("Func:store_hid_client_char_descriptors(),UnKnown UUID \n"));
        }
        break;
    }
}
             ...
             const GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM_T *const cfm )
{
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_read_char_value_cfm(), Entry\n"));
    ...
    if(CHECK_GATT_SUCCESS(cfm->status) && (hid_instance))
    {
        ...
        for(count = 0;count < hid_instance->num_report_ids;count++)
        {
            ...
            if(report_id_map->report_reference_handle == cfm->handle)
            {
                ...
                GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_read_char_value_cfm(), ID: %x, Type: %x\n",report_id_map->report_id,report_id_map->type ));
            }
        }
                ...
                const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTIC_DESCRIPTORS_CFM_T *const cfm)
{
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(), Entry\n"));
    ...
    if((CHECK_GATT_SUCCESS(cfm->status) || CHECK_DESCRIPTOR_NOT_MANDATORY((uint16)cfm->uuid[0])) && (hid_instance))
    {
        ...
        if(!cfm->more_to_come)
        {
             ...
             if(hid_instance->char_descriptor_mask)
            {
                ...
                GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(), Find Next\n"));
                discovery_pending = hid_discover_next_char_descriptor(hid_client,hid_instance,cfm->cid);
            }
        }
        GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(), discovery_pending : %d, mask = %d\n",discovery_pending,hid_instance->char_descriptor_mask));
        /* Check Discovery is pending */
        if(!discovery_pending) 
        {
            /* Read report ID's */
            GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(),NumID = %d \n",hid_instance->num_report_ids));
            for(count = 0;count < hid_instance->num_report_ids;count++)
            {
                /* Find out report id map instance */
                report_id_map = (hid_client->report_id_map )+(count);
                GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(),: FindreportId: handle = %x \n",report_id_map->report_reference_handle));
            ...
            {
                GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_descriptors_cfm(), Send Inti CFM to app\n"));
                hid_client_send_discovery_cfm(hid_client,hid_instance,cfm->cid,gatt_hid_client_status_success);
            }
        }
    }
               ...
               const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T *const cfm)
{
    ...
    if(CHECK_GATT_SUCCESS(cfm->status) && (hid_instance))
    {
        if(cfm->uuid_type == gatt_uuid16)
        {
            ...
            if(!cfm->more_to_come)
            {
                ...
                if(hid_instance->char_descriptor_mask)
                {
                    ...
                    GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_cfm(), Find descriptors \n"));
                    discovery_pending = hid_discover_next_char_descriptor(hid_client,hid_instance,cfm->cid);
                }
            }
        }  
        ...
        if(!discovery_pending)
        {
            GATT_HID_CLIENT_DEBUG_INFO(("Func:handle_hid_client_discover_all_char_cfm(), Discovery Completed \n"));
...
void  hid_client_start_discovery(GHIDC_T *const hid_client,
       const  HID_CLIENT_INTERNAL_MSG_DISCOVER_T *msg)
{
        GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_start_discovery(), Entry\n"));

        /* Set Discovery is in progress */
        ...
        uint16 end_handle)
{
    ...
    if(cfg == NULL)
    {
        ...
        hid_discovery_instance = malloc(hid_instance_size);
        if(hid_discovery_instance == NULL)
            return FALSE;
        memset(hid_discovery_instance,0,hid_instance_size);

        GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_init_discovery_instance(), Config Defualt values\n"));
         /*Set Default Values */
    ...
    hid_discovery_instance->characterisitc_handle = INVALID_HID_HANDLE;
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_init_discovery_instance(),End handle %x\n",end_handle));
    hid_discovery_instance->end_handle = end_handle;
    GATT_HID_CLIENT_DEBUG_INFO(("Func:hid_client_init_discovery_instance(), Exit\n"));
