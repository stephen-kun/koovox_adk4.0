...
static void deviceInfoClientConnect(GDISC *const device_info_client, 
                const DEVICE_INFO_CLIENT_INTERNAL_MSG_CONNECT_T *connect_req)
{
        ...
        if(char_cfm->status == gatt_status_success)
        {
            ...
            if(char_cfm->uuid_type == gatt_uuid16)
            {
                switch(char_cfm->uuid[0])
                {
                    ...
                    default:
                        GATT_DEVICE_INFO_DEBUG_INFO(("GATT: Invalid characteristic Handle received for Device Info Service\n"));
                        break;
               }
                ...
                const GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM_T *read_cfm)
{
    ...
    if(device_info_client != NULL)
    {
        GATT_DEVICE_INFO_DEBUG_INFO(("handleDeviceInfoReadCharResp (Size_Value= %d, Status = %d)\n",read_cfm->size_value, read_cfm->status));
        ...
        sendDeviceInfoClientReadCharCfm(device_info_client, status, (GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM_T *)read_cfm);
    }
}
/****************************************************************************/
static void handleGattMsg(Task task, MessageId id, Message payload)
{
    ...
    GATT_DEVICE_INFO_DEBUG_PANIC(("Client Gatt Msg not handled\n"));
}
/****************************************************************************/
static void handleGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    GATT_DEVICE_INFO_DEBUG_INFO(("handleGattManagerMsg (ID= %x)\n",id));
    ...
    if(device_info_client != NULL)
    {
        switch (id)
        {
            ...
            {
                handleDeviceInfoDiscoverCharCfm(device_info_client, (GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T *)payload);
            }
            break;

            case GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM:
            {
                handleDeviceInfoReadCharResp(device_info_client, (GATT_MANAGER_READ_CHARACTERISTIC_VALUE_CFM_T *)payload);
            }
            break;
            ...
            {
                /* Unrecognised GATT message */
                GATT_DEVICE_INFO_DEBUG_PANIC(("Client GattMgr Msg not handled\n"));
            }
            break;
        }
    }
...
static void handleInternalDeviceInfoMsg(Task task, MessageId id, Message msg)
{
    GDISC *device_info_client = (GDISC *)task;
     ...
     GATT_DEVICE_INFO_DEBUG_INFO(("handleInternalDeviceInfoMsg (ID= %x)\n",id));
    ...
    if(device_info_client != NULL)
    {
        switch (id)
        {
            ...
            {
                deviceInfoClientConnect(device_info_client, (DEVICE_INFO_CLIENT_INTERNAL_MSG_CONNECT_T*)msg);
            }
            break;
            
            case DEVICE_INFO_CLIENT_INTERNAL_MSG_READ_CHAR:
            {
                deviceInfoReadCharRequest(device_info_client, (DEVICE_INFO_CLIENT_INTERNAL_MSG_READ_CHAR_T*)msg);
            }
            break;
            ...
            default:
            {
                ...
                GATT_DEVICE_INFO_DEBUG_PANIC(("Client Internal Msg not handled\n"));
            }
            break;
        }
    }
...
{
     GATT_DEVICE_INFO_DEBUG_INFO(("deviceInfoClientMsgHandler (ID= %x)\n",id));
