...
static void csrA2dpDecoderPluginSendLlDspGain(uint16 master, uint16 tone)
{
    ...
    KalimbaSendMessage(LOW_LATENCY_VOLUME_MSG, 0, digital_dsp, master, tone);
}
...
static void csrA2dpDecoderPluginSetLlHwGain(uint16 master)
{
    if (CsrMultiChanGainGetType(multi_channel_group_main) == multi_channel_gain_digital)
    {
        ...
        CsrMultiChanGainSetHardware(multi_channel_group_main, master, NULL);
    }
    else
    {
        /* Hybrid/Hardware mode - set the gain in hardware only */
        CsrMultiChanGainSetHardwareOnly(multi_channel_group_main, master);
    }
}
               ...
               case MUSIC_TONE_COMPLETE:
                ...
                case LOW_LATENCY_CODEC_MSG:
                {
                    ...
                    {  
                        ...
                        csrA2dpDecoderPluginSetLlHwGain(m->a);
                    }
                }
                break;
...
AUDIO_OUTPUT_TYPE_T CsrA2dpDecoderPluginIfLowLatencyGetOutputType(void)
{
    if(isCodecLowLatencyBackChannel())
    {
        return CsrMultiChanGetOutputType(mch_out_primary_left);
    }
