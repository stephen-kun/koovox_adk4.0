...
/*!
    @brief Status code returned from the GATT immediate alert service server library
    This status code indicates the outcome of the request.
*/

typedef enum
{
    gatt_imm_alert_server_status_success,                  /* Requested operation was successful */
    gatt_imm_alert_server_status_registration_failed,   /* Registration operation to GATT manager was failed  */
    gatt_imm_alert_server_status_invalid_parameter
}gatt_imm_alert_server_status;
...
/*!
    ...
    @return The status result of calling the API as defined in gatt_imm_alert_server_status.
