                                    ...
                                    const uint8 *value)
{
    if (!GattManagerServerAccessResponse(task, cid, handle, result, size_value, value))
    {
        ...
        GATT_DEVICE_INFO_DEBUG_PANIC(("Couldn't send GATT access response\n"));
    }
}
...
static void handleDeviceInfoReadRequest(const gdiss_t *dev_info_server, const GATT_MANAGER_SERVER_ACCESS_IND_T *access_ind)
{
    ...
    sendDeviceInfoAccessRsp((Task)&dev_info_server->lib_task, access_ind->cid, access_ind->handle, result, size, value);
}
...
{
    GATT_DEVICE_INFO_DEBUG_INFO((" Func:handleDeviceInfoServerAccess(), Access Ind flags = %x \n",access_ind->flags));
