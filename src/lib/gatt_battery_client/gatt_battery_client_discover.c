...
static void nextAfterDiscoverCharacteristics(GBASC *battery_client)
{
    switch (battery_client->pending_cmd)
    {
        ...
        case battery_pending_set_notify_disable:
        {
            ...
            batterySetNotificationRequest(battery_client, (battery_client->pending_cmd == battery_pending_set_notify_enable) ? TRUE : FALSE, FALSE);
        }
        break;
        ...
        default:
        {
            GATT_BATTERY_DEBUG_PANIC(("BASC: No action after discover characteristics [0x%x]\n", battery_client->pending_cmd));
        }
        break;
    }
}
    ...
    switch (battery_client->pending_cmd)
    {
        ...
        case battery_pending_set_notify_disable:
        {
            if (cfm->uuid_type == gatt_uuid16)
            {
                ...
                {
                    writeClientConfigValue(battery_client, (battery_client->pending_cmd == battery_pending_set_notify_enable) ? TRUE : FALSE, cfm->handle);
        ...
        default:
        {
            GATT_BATTERY_DEBUG_PANIC(("BASC: Processing descriptor in wrong state [0x%x]\n", battery_client->pending_cmd));
        }
        break;
    }
}
...
static void nextAfterDiscoverDescriptors(GBASC *battery_client)
{
    switch (battery_client->pending_cmd)
    {
        ...
        default:
        {
            GATT_BATTERY_DEBUG_PANIC(("BASC: No action after discover descriptors [0x%x]\n", battery_client->pending_cmd));
        }
        break;
    }
}
...
void handleDiscoverAllCharacteristicsResp(GBASC *battery_client, const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T *cfm)
{
    if (cfm->status == gatt_status_success)
    {
        ...
        */
        if (battery_client->handle_level_start && (cfm->handle > battery_client->handle_level_start) && (cfm->handle < battery_client->handle_level_end))
        {
