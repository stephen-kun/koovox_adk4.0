        ...
        )
{
    ...
    /* Try and get the streams, increase sink_list size until it is big
       ...
       TODO: B-170504 - will be updated when StreamsSinksFromBdAddr accepts
             tp_bdaddr instead of typed_bdaddr.
     */
    for (
        max_sinks = 5;
        (sink_list = (Sink *)PanicNull(calloc(max_sinks, sizeof(Sink)))) &&
        !StreamSinksFromBdAddr(&max_sinks, sink_list, &tpaddr);
        max_sinks += 5
        )
    {
        free(sink_list);
...
void connectionHandleEncryptionChange(DM_SM_ENCRYPTION_CHANGE_IND_T *ind)
{
    ...
    /* Try and get the streams, increase sink_list size until it is big
       enough or a panic occurs.
       TODO: B-170504 - StreamSinksFromBdaddr needs updating for pt_addr.
     */
    for (
        max_sinks = 5;
        (sink_list = (Sink *)PanicNull(calloc(max_sinks, sizeof(Sink)))) &&
