...
static void sendVolumeMessage(const uint16 volume)
{
    ...
    if(!KalimbaSendMessage(MESSAGE_EXAMPLE_PLUGIN_VOLUME, 0, digital_dsp, volume, CSR_COMMON_EXAMPLE->tone_volume))
    {
        PRINT(("CSR_COMMON_EXAMPLE: Message MESSAGE_EXAMPLE_PLUGIN_VOLUME failed!\n"));
        Panic();
    }
}
...
static void connectAudio(const ExamplePluginTaskdata * const task)
{
    ...
    if(CSR_COMMON_EXAMPLE->audio_sink)
    {
        ...
        if(!KalimbaSendMessage(MESSAGE_SET_DAC_SAMPLE_RATE,
                               params.sample_rate / 10, params.sample_rate/10,
                               0, 0))
        {
            PRINT(("CSR_COMMON_EXAMPLE: Message MESSAGE_SET_DAC_SAMPLE_RATE failed!\n"));
            Panic();
        }
...
{    
    ...
    KalimbaSendMessage(MESSAGE_SETMODE, SYSMODE_PSTHRGH, 0, 0,
